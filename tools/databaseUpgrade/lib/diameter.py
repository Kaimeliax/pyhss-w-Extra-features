...
class Diameter:
    ...
    def Answer_16777251_316(self, packet_vars, avps):
        ...
        def determine_access_restriction(subscriber_data, roaming_network, mcc, mnc):
            
            #Determines if the subscriber is roaming or at home, and applies the appropriate access restriction.
            #Returns the final access restriction value for use in subscription data AVPs.
            #- subscriber_data: Dictionary containing 'home_access_restriction' and 'roaming_access_restriction' for the subscriber.
            #- roaming_network: Instance of ROAMING_NETWORK or similar structure with 'access_restriction_data' for roaming.
            #- mcc, mnc: Current MCC and MNC values of the visited network.
            # Check if the subscriber is roaming
            # is_roaming = not (mcc == HOME_MCC and mnc == HOME_MNC)
            
            if subscriberIsRoaming:
                # Retrieve the subscriber's roaming restriction and the partner's restriction from the roaming network
                subscriber_roaming_restriction = subscriber_data.get('roaming_access_restriction', '00000050')
                partner_restriction = getattr(roaming_network, 'access_restriction_data', '00000050')
                
                # Apply the higher restriction value between the subscriber's roaming restriction and the partner's restriction
                final_access_restriction = max(subscriber_roaming_restriction, partner_restriction)
            else:
                # Apply the home access restriction if the subscriber is not roaming
                final_access_restriction = subscriber_data.get('home_access_restriction', '00000050')
            
            return final_access_restriction  # Return the final access restriction for AVP use

        # Generate subscription data with Access-Restriction-Data and other required AVPs
        subscription_data = ''
        subscription_data += self.generate_vendor_avp(1426, "c0", 10415, final_access_restriction)  # Access Restriction Dat
        ...
        for apn_id in apn_list:
            ...
            # Apply QCI override
            qci = self.database.apply_qci_override(subscriber_details, apn_data['apn'], subscriber_details['mcc'], subscriber_details['mnc'])
            AVP_QoS = self.generate_vendor_avp(1028, "c0", 10415, self.int_to_hex(qci, 4))
