...
class SUBSCRIBER(Base):
    ...
    home_access_restriction_data = Column(String(8), default='00000050', doc='Home Access Restriction Data (e.g., restricts 5G NSA (00000050)')
    roaming_access_restriction_data = Column(String(8), default='00000050', doc='Roaming Access Restriction Data for roaming (e.g., restricts 5G NSA when roaming (00000050)')
    qci = Column(Integer, default=9, doc='Default QCI value for home network')
...
class ROAMING_NETWORK(Base):
    ...
    roamer_access_restriction_data = Column(String(8), default='00000050', doc='Access restriction data for roaming network')
    roamer_qci = Column(Integer, default=9, doc='Default QCI value for roaming networks on other APNs not shown in roamer_apm_specific_qci')
    roamer_apn_specific_qci = Column(String(512), doc='Comma-separated APN to QCI mapping, e.g., "internet:9,data:8"')  # Optional field
    last_modified = Column(String(100), default=datetime.datetime.now(tz=timezone.utc), doc='Timestamp of last modification')
...
class Database:
    ...
    def add_roaming_network(self, roaming_network_data):
        session = self.Session()
        try:
            new_roaming_network = ROAMING_NETWORK(**roaming_network_data)
            session.add(new_roaming_network)
            session.commit()
        except Exception as e:
            session.rollback()
            print(f"Error adding roaming network: {e}")
        finally:
            session.close()
    ...
    def get_subscriber(self, imsi):
        session = self.Session()
        try:
            subscriber = session.query(SUBSCRIBER).filter_by(imsi=imsi).first()
            return subscriber
        except Exception as e:
            print(f"Error retrieving subscriber: {e}")
        finally:
            session.close()

    def get_roaming_network(self, name):
        session = self.Session()
        try:
            roaming_network = session.query(ROAMING_NETWORK).filter_by(name=name).first()
            return roaming_network
        except Exception as e:
            print(f"Error retrieving roaming network: {e}")
        finally:
            session.close()

    def update_subscriber(self, imsi, update_data):
        session = self.Session()
        try:
            subscriber = session.query(SUBSCRIBER).filter_by(imsi=imsi).first()
            if subscriber:
    ...
    def update_roaming_network(self, name, update_data):
        session = self.Session()
        try:
            roaming_network = session.query(ROAMING_NETWORK).filter_by(name=name).first()
            if roaming_network:
                for key, value in update_data.items():
                    setattr(roaming_network, key, value)
                session.commit()
        except Exception as e:
            session.rollback()
            print(f"Error updating roaming network: {e}")
        finally:
            session.close()
    ...
    def delete_roaming_network(self, name):
        session = self.Session()
        try:
            roaming_network = session.query(ROAMING_NETWORK).filter_by(name=name).first()
            if roaming_network:
                session.delete(roaming_network)
                session.commit()
        except Exception as e:
            session.rollback()
            print(f"Error deleting roaming network: {e}")
        finally:
            session.close()
